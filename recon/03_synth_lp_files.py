...
def df_mem_info(what,df):
    ...
    print("Computed memory {} memory usage: {:,}".format(what,total_size(df)))
    print("==============================")
...
def update_constraints(f, level, n_con, summary_nums, geo_id, state_code, county, state_abbr):
    logging.info(f"START {state_code}{county} {level} {n_con} {geo_id}")
    tuple_list=[]
    constraint_list=[]
    ...
    if args.mem:
        df_mem_info("tuple_frame",tuple_frame)
        df_mem_info("con_frame",con_frame)
        df_mem_info('lp',lp)
    del tuple_frame
    del con_frame
    gc.collect()
    ...
    if args.mem:
        df_mem_info("lp_short",lp_short)
        df_mem_info("lp_df",lp_df)
    del lp_short
    gc.collect()
    ...
    del lp_df
    gc.collect()
    logging.info(f"END {state_code}{county} {level} {n_con} {geo_id}")
    return n_con
...
class LPTractBuilder:
    ...
    def get_constraint_summary(self, level, p01_data, data_dict, summary_nums):
        ...
        logging.info("END {}: processed data_dict (len={:,}) len(self.master_tuple_list)={:,}".
                     format(level,len(data_dict),len(self.master_tuple_list)))
    ...
    def build_tract_lp(self,state_abbr, county, tract, sf1_tract_data, sf1_block_data):
        ...
        if args.mem:
            ...
            ET.SubElement(dfxml.doc, 'synth_lp_files').text = 'success'
            exit(0)
...
if __name__=="__main__":
    ...
    parser.add_argument("--j1", help="Specifies number of threads for state-county parallelism. These threads do not share memory. Specify 1 to disable parallelism.",
                        type=int,default=4)
    parser.add_argument("--j2", help="Specifies number of threads for tract-level parallelism. These threads largely share memory. Specify 1 to disable parallelism.",
                        type=int,default=16)
    parser.add_argument("--dry_run", help="don't actually write out the LP files",action='store_true')
    parser.add_argument("--mem", help="enable memory debugging. Print memory usage. Write output to temp file and compare with correct file.", action='store_true')
    ...
    dbrecon.setup_logging(config=config,args=args,prefix="03syn")
